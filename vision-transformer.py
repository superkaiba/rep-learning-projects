...
class LayerNorm(nn.Module):
    ...
    def forward(self, inputs):
        ...
        """
        mean = torch.mean(inputs, dim=-1).unsqueeze(-1)
        stdev = torch.sqrt(torch.var(inputs, dim=-1, unbiased=False) + self.eps).unsqueeze(-1) # Biased version

        x_hat = (inputs - mean)/stdev
...
class MultiHeadedAttention(nn.Module):
    def __init__(self, head_size, num_heads, sequence_length):
        ...
        self.weight_size = (self.num_heads * self.head_size + 1, self.num_heads * self.head_size) # +1 for bias

        self.W_Q = torch.nn.Parameter(torch.zeros(size=self.weight_size, requires_grad=True))
        self.W_K = torch.nn.Parameter(torch.zeros(size=self.weight_size, requires_grad=True))
        self.W_V = torch.nn.Parameter(torch.zeros(size=self.weight_size, requires_grad=True))
        self.W_O = torch.nn.Parameter(torch.zeros(size=self.weight_size, requires_grad=True))
    ...
    def get_attention_weights(self, queries, keys):
        ...
        """
        dot_products = torch.matmul(queries, keys.transpose(-2, -1))/math.sqrt(self.head_size)
        probs = F.softmax(dot_products, dim=3)
        return probs
    ...
    def apply_attention(self, queries, keys, values):
        ...
        total_values = torch.matmul(attention_weights, values)
        return self.merge_heads(total_values)
    ...
    def split_heads(self, tensor):
        ...
        reshaped = tensor.reshape(tensor.shape[0], tensor.shape[1], self.num_heads, int(tensor.shape[2]/self.num_heads))
    ...
    def merge_heads(self, tensor):
        ...
        reshaped = transposed.reshape(transposed.shape[0], transposed.shape[1], self.num_heads * transposed.shape[3])
        return reshaped
    ...
    def forward(self, hidden_states):
        ...
        """
        X = torch.cat([hidden_states, torch.ones(size=(*hidden_states.shape[:-1], 1))], dim=2).unsqueeze(-2)
        Q = torch.matmul(X, self.W_Q).squeeze(-2) 
        K = torch.matmul(X, self.W_K).squeeze(-2) 
        V = torch.matmul(X, self.W_V).squeeze(-2) 

        hidden_states = self.apply_attention(self.split_heads(Q), self.split_heads(K), self.split_heads(V))
        Y = torch.cat([hidden_states, torch.ones(size=(*hidden_states.shape[:-1], 1))], dim=2).unsqueeze(-2)
        output = torch.matmul(Y, self.W_O).squeeze(-2)
        return output
